...
# adding the parent directory 10i
# the sys.path.
...
def par_convergence(model, parameters, output_file, cpus=18):
    for j, params in enumerate(parameters):
        print(f'Started simulation {j} with params {params}')
        
        if '-g' in params:            
            print(f'Simulating with {cpus} CPU workers')
            start = time.time()
            values = annual_irradiance(model, WEA, params, cpus)
            end = time.time()
            values += [end-start]
        else:
            # continue
