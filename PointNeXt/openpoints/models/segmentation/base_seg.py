...
class SegHead(nn.Module):
    def __init__(self,
        ...
        if kwargs:
            if not (len(kwargs) == 1 and 'linear_channels' in kwargs.keys()):
                logging.warning(f"kwargs: {kwargs} are not used in {__class__.__name__}")
        ...
        if 'linear_channels' in kwargs.keys():
            linear_block = nn.ModuleList([])
            
            if len(kwargs['linear_channels']) > 1:
                for i, channel in enumerate(kwargs['linear_channels'][:-1]):
                    layer = nn.Linear(channel, kwargs['linear_channels'][i+1])
                    linear_block.append(layer)
                    
            elif len(kwargs['linear_channels']) == 1:
                layer = nn.Linear(kwargs['linear_channels'][0], kwargs['linear_channels'][0])
                linear_block.append(layer)
            else:
                pass
            
            for layer in linear_block:
                self.head = nn.Sequential(self.head, layer)
